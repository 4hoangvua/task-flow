...
- [x] Khắc phục lỗi thiếu biến môi trường DATABASE_URL trên máy chủ sản phẩm khiến PM2 restart loop (2026-05-27)
- [x] Cài đặt lại địa chỉ IP AWS mới 3.239.239.72 cho backend, frontend local, Nginx và CORS trên server (2026-05-27)
- [x] Khắc phục lỗi đăng nhập 401 do cơ sở dữ liệu trống trên server và thực hiện chạy seed thành công (2026-05-28)
- [x] Khắc phục lỗi bảo mật active status, unassign công việc khi thành viên bị xóa, đồng bộ validation hạn chót, hiển thị chi tiết lỗi nghiệp vụ, chặn kéo thả DONE khi thiếu prerequisite, tối ưu chọn dự án ở Dashboard (2026-05-28)
- [x] Di chuyển cơ sở dữ liệu sang Neon PostgreSQL, cấu hình dynamic schema test isolation, đồng bộ hóa & seed thành công và vượt qua 100% bộ kiểm thử tự động (2026-05-28)
...
- **Địa chỉ IP Server:** `3.239.239.72`
